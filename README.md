...
chmod +x ./k8s/install-mandatory-services.sh
...
# Install
ENV:
- dev: keycloak in develop mode. TLS with autocertificate (domain is not need)
- prod: keycloak in prod mode. TLS with letsencrypt (domain is need)
```
export DASHBOARD_CN=devtools
export BASE_DNS=example.com
export ADMIN_EMAIL=admin@example.com
export ENV=prod | dev
./k8s/install-mandatory-services.sh
```

# After install
If you use minikube, execute `minikube ip` and add
```
192.168.49.2      devtools.192.168.49.2.nip.io keycloak.192.168.49.2.nip.io
```
...
helm uninstall dashboard-frontend -n dev-tools
helm uninstall dashboard-backend -n dev-tools
./k8s/dashboard-frontend/install.sh
./k8s/dashboard-backend/install.sh

# licencias
- Este proyecto incluye TailwindCSS, que está licenciado bajo la licencia MIT.
Ver dashboard-installer/src/templates/lib/LICENSE para más detalles.  