...
if ! minikube tunnel --cleanup >/dev/null 2>&1; then
  echo "[WARNING] Minikube tunnel is not running. Please start it with 'minikube tunnel' to expose services."
else
  echo "Minikube tunnel is running."
fi
if ! nc -z host.docker.internal 8001; then
  echo "[WARNING] kubectl proxy is not running. Start it with:"
  echo "kubectl proxy --address=0.0.0.0 --disable-filter=true"
else
  echo "kubectl proxy is active on host.docker.internal:8001"
fi