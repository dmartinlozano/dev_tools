from tools import config
import time
import subprocess
import base64
import os
from pathlib import Path
from kubernetes.stream import stream
from kubernetes import client
from kubernetes.client.rest import ApiException

core_api = client.CoreV1Api()
rbac_api = client.RbacAuthorizationV1Api()

def get_dns():
    kubeconfig_path = os.path.expanduser("~/.kube/config")
    if os.path.isfile(kubeconfig_path):
        with open(kubeconfig_path, "r") as f:
            if "minikube.sigs.k8s.io" in f.read():
                import subprocess
                try:
                    ip = subprocess.check_output(["minikube", "ip"]).decode().strip()
                    return f"{ip}.nip.io"
                except Exception:
                    pass
            #TODO else without minikube
    return ""

def create_namespace():

    namespaces = core_api.list_namespace()
    namespace_exists = False

    for ns in namespaces.items:
        if ns.metadata.name == config.namespace:
            namespace_exists = True
            
    if namespace_exists:
        print(f"Namespace '{config.namespace}' already exists")
    else:
        print(f"Namespace '{config.namespace}' not found. Creating it...")
        ns_body = client.V1Namespace(metadata=client.V1ObjectMeta(name=config.namespace))
        core_api.create_namespace(body=ns_body)
        print(f"Namespace '{config.namespace}' created successfully")

def get_secret( secret_name):
    try:
        secret = core_api.read_namespaced_secret(name=secret_name, namespace=config.namespace)
        for key in secret.data:
            secret.data[key] = base64.b64decode(secret.data[key]).decode('utf-8')
        return secret.data
    except ApiException as e:
        if e.status == 404:
            print("Secret not found")
            return None
        else:
            raise
    except Exception as e:
        return None

def create_secret(secret_name, data):
    secret_body = client.V1Secret(
        metadata=client.V1ObjectMeta(name=secret_name),
        data=data,
        type="Opaque",
    )
    try:
        core_api.create_namespaced_secret(namespace=config.namespace, body=secret_body)
        print("Secret created")
    except ApiException as e:
        if e.status == 409:
            core_api.replace_namespaced_secret(name=secret_name, namespace=config.namespace, body=secret_body)
            print("Secret replaced")
        else:
            raise

def wait_for_deployment_available(deployment_name, timeout=300):
    apps_api = client.AppsV1Api()
    start = time.time()
    while True:
        try:
            dep = apps_api.read_namespaced_deployment(deployment_name, config.namespace)
            for cond in dep.status.conditions or []:
                if cond.type == "Available" and cond.status == "True":
                    print(f"{deployment_name} is available")
                    return True
        except ApiException as e:
            if e.status != 404:
                print(f"Error: {e}")
        if time.time() - start > timeout:
            raise TimeoutError(f"Timeout waiting for deployment {deployment_name} to be available")
        time.sleep(3)

def apply_kubectl(file_path):
    print(f"Applying {file_path}...")
    cmd = ["kubectl", "apply", "-f", file_path, "--namespace", config.namespace]
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode != 0:
        raise RuntimeError(result.stderr)

def exec_command(pod_name, command, env=None):
    if env:
        env_str = " ".join(f"{k}={v}" for k, v in env.items())
        full_command = f"{env_str} {command}"
    else:
        full_command = command
    exec_command = [
        '/bin/sh',
        '-c',
        full_command
    ]
    return stream(
        core_api.connect_get_namespaced_pod_exec,
        pod_name,
        config.namespace,
        command=exec_command,
        stderr=True,
        stdin=False,
        stdout=True,
        tty=False,
    )

def exec_script(script, params=None):
    script_path = Path(__file__).parent / script
    print(f"Running {script_path} ...")
    cmd = ["bash", str(script_path)]
    if params:
        cmd += [str(p) for p in params]
    try:
        subprocess.run(
            cmd,
            stdout=None,
            stderr=True,
            check=True
        )
    except subprocess.CalledProcessError as e:
        print(f"Error to execute {script_path}. Error: {e.stderr}")
        raise RuntimeError(f"Error to execute {script_path}: {e.stderr}")
    
def exec_script_in_pod(pod_name, script, params=None, namespace="dev-tools"):
    script_path = Path(__file__).parent / script
    remote_path = f"/tmp/{Path(script).name}"
    print(f"Copying {script_path} to pod {pod_name}:{remote_path} ...")
    subprocess.run([
        "kubectl", "cp", str(script_path), f"{namespace}/{pod_name}:{remote_path}"
    ], check=True)
    cmd = ["kubectl", "exec", pod_name, "-n", namespace, "--", "bash", remote_path]
    if params:
        cmd += [str(p) for p in params]
    print(f"Running script in pod: {' '.join(cmd)}")
    result = subprocess.run(cmd, capture_output=True, text=True)
    print("[STDOUT]", result.stdout)
    if result.returncode != 0:
        print("[STDERR]", result.stderr)
        result.check_returncode()
    return result.stdout
    
def wait_for_service_running(service, max_retries=30, retry_interval=10):
    core_api = client.CoreV1Api()
    retry_count = 0
    while retry_count < max_retries:
        retry_count += 1
        print(f"      Try {retry_count} of {max_retries}...")
        try:
            pods = core_api.list_namespaced_pod(
                namespace=config.namespace,
                label_selector=f"app.kubernetes.io/name={service}"
            ).items
        except ApiException as e:
            pods = []
        if not pods:
            time.sleep(retry_interval)
            continue
        pod_status = pods[0].status.phase
        if pod_status == "Running":
            print(f"    {service} pod is Running.")
            break
        else:
            time.sleep(retry_interval)
    else:
        print(f"{service} was not Running after waiting {max_retries * retry_interval} seconds.")
        try:
            pod_name = pods[0].metadata.name if pods else None
            if pod_name:
                desc = core_api.read_namespaced_pod(name=pod_name, namespace=config.namespace)
                print(desc)
        except Exception as e:
            print(f"Error describing pod: {e}")
            raise RuntimeError(f"{service} was not Running after waiting {max_retries * retry_interval} seconds.")
