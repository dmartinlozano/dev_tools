...
from tools.postgresql.utils import create_database
from tools.helm_client import HelmClient
from tools.utils import exec_script_in_pod, get_secret, wait_for_service_running
...
class Keycloak:
    ...
    def install(self):
        ...
        db_user, db_password = create_database("keycloak")
        ...
        if not admin_password:
            admin_password = config_data["adminPassword"]
        ...
        create_credentials(vault_path="keycloak/admin", username="admin", password=admin_password)  


        # wait for keycloak api to be available
        print("Waiting configuring keycloak...")
        output = exec_script_in_pod(
            pod_name="keycloak-0",
            script="keycloak/scripts/configure.sh",
            params=[
                admin_password
            ]
        )
    ...
    def create_keystore(self):
        ...
        # extract secrets
        secret = core_api.read_namespaced_secret("services-tls", config.namespace)
        ...
        core_api.replace_namespaced_secret("services-tls", config.namespace, secret)
