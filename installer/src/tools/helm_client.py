...
class HelmClient:
    ...
    def install(self, name, location, values_file=None, extra_env_vars=None, values=None):
        cmd = ["helm", "upgrade", "--install", name, location, "--namespace", config.namespace]
        if values_file:
            cmd += ["-f", values_file]
        if extra_env_vars:
            env_yaml = {
    ...
    #     cmd = ["helm", "uninstall", release, "--namespace", config.namespace]
    #     return self._run(cmd)

    # def get_status(self, release):
    #     cmd = ["helm", "status", release, "--namespace", config.namespace]
    #     return self._run(cmd)

    def list_releases(self):
        cmd = ["helm", "list", "--namespace", config.namespace, "--output", "json"]
