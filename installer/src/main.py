import config #Dont delete
from helm_client import HelmClient
from dev_tools_deployer import DevToolsDeployer
from utils import create_namespace, create_secret, get_secret
...
def index():
    ...
    with ui.card().classes('fixed-center'):
        ...
        def on_next():
            ...
            elif devToolsDeployer.mandatory_tools_step == 0:
                ui.navigate.to('/mandatory-tools-initial-installation')
            elif devToolsDeployer.mandatory_tools_step == 1:
...
def mandatory_tools_initial_installation():
    ...
    with ui.card().style('min-width: 420px; max-width: 600px; width: 100%;').classes('fixed-center'):
        ...
        def submit():
            ...
            devToolsDeployer.mandatory_tools_step = 1
            ui.navigate.to('/mandatory-tools-progress-installation')
...
def mandatory_tools_progress_installation():
    set_background()
    if not devToolsDeployer.is_installing:
        threading.Thread(target=devToolsDeployer.install, daemon=True).start()
    with ui.card().style('min-width: 420px; max-width: 600px; width: 100%;').classes('fixed-center'):
        ui.label('Installing DevTools').style('font-size: 1.5rem; font-weight: bold; text-align: center; width: 100%;')
        progress_label = ui.label(devToolsDeployer.progress_steps[-1] if devToolsDeployer.progress_steps else 'Starting...').classes('mb-2')
        ...
        def update_progress():
            if devToolsDeployer.progress_steps:
                progress_label.set_text(devToolsDeployer.progress_steps[-1])
            if devToolsDeployer.is_installing:
